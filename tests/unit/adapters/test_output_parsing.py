...
@pytest.mark.parametrize(
    "adapter_fixture", ["mock_guardrails_adapter", "mock_llama_guard_adapter", "mock_nemo_adapter"]
)
def test_audit_structure_invariants(adapter_fixture, request):
    ...
    if audit.get("response_hash"):
        assert sha256_pattern.match(audit["response_hash"]), (
            f"{adapter.name}: invalid response_hash"
        )
...
ADVERSARIAL_METADATA_CASES = [
    ...
    {
        "meta": {"prompt": "secret prompt", "content": {"text": "hidden"}},
        "leak_terms": ["secret prompt", "hidden"],
    },
    # Case 2: Large text fields (>128 chars)
    {"meta": {"log": "x" * 500}, "leak_terms": ["x" * 500]},
    # Case 3: Custom vendor fields (e.g., input_text)
    {
        "meta": {"input_text": "PII: email@test.com", "custom_prompt": "leak"},
        "leak_terms": ["email@test.com", "leak"],
    },
...
@pytest.mark.parametrize(
    "case", ADVERSARIAL_METADATA_CASES, ids=lambda c: f"leak_{c['leak_terms'][0][:20]}"
)
def test_metadata_scrubbing_adversarial(adapter_fixture, case, request):
    ...
    for term in leak_terms:
        assert term not in audit_str, f"{adapter.name}: metadata leaked '{term[:50]}' in audit"
