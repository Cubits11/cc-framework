...
\begin{hypothesis}[H1: Complementarity threshold evidence (harmful / AND)]
\label{hyp:H1-final}
...
and reject $H_{1,0}$---thereby reporting finite-sample evidence for
the pre-registered complementarity threshold for this pair---if and only if
\begin{equation}
  U^{(\lambda)}_1 \le \tau_1.
...
declare threshold evidence whenever the lower confidence limit for $s_1$ exceeds
$s_1^\star$.
\end{hypothesis}

\begin{hypothesis}[H2: Benign-inflation threshold evidence (benign / OR)]
\label{hyp:H2-final}
...
$H_{2,0}$---reporting finite-sample evidence for acceptable benign inflation---if and
only if
\begin{equation}
  U^{(\lambda)}_0 \le \tau_0.
...
threshold $s_0^\star = 1-\tau_0$, we declare threshold evidence whenever the lower
