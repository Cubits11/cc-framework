...
def _plot_phase_surface(
    E: np.ndarray, T: np.ndarray, C: np.ndarray, last_eps: float, last_T: float, fig_path: Path
) -> None:
...
def plot_fh_heatmap(
    ...
    *,
    ...
    ax.set_xlabel("Index in ROC_B")
    ax.set_ylabel("Index in ROC_A")
    ax.set_title(f"FH Envelope on Composed J [{comp}]")
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label("FH ceiling on composed J")
    _save_opaque(fig, outpath)
    plt.close(fig)
...
def _plot_cc_ci_figure(
    *,
    ...
    ax.errorbar(
        x=cc_hat,
        y=0,
        xerr=[[cc_hat - bern_lo], [bern_hi - cc_hat]],
        fmt="o",
        color=CBLUE,
        capsize=4,
        lw=2.0,
    ...
    ax.errorbar(
        x=cc_hat,
        y=1,
        xerr=[[cc_hat - newc_lo], [newc_hi - cc_hat]],
        fmt="o",
        color=CRED,
        capsize=4,
        lw=2.0,
        label="Newcombe (Wilson)",
    )
...
def plot_cc_ci_comparison(
    *,
    ...
    _plot_cc_ci_figure(
        cc_hat=cc_hat,
        bern_lo=float(bern_lo),
        bern_hi=float(bern_hi),
        newc_lo=float(newc_lo),
        newc_hi=float(newc_hi),
        outpath=outpath,
        title=title,
    )
...
def _add_week3_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--week3-theta", type=str, default=None, help="θ as 'tpr_a,fpr_a,tpr_b,fpr_b'")
    ap.add_argument("--week3-I1", type=str, default=None, help="I1 interval as 'lo,hi'")
    ap.add_argument("--week3-I0", type=str, default=None, help="I0 interval as 'lo,hi'")
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    if E.size >= 10:
        _save_opaque(
            fig := plt.figure(figsize=(1, 1)), fig_dir / "phase_diagram.pdf"
        )  # placeholder if any side effects
        plt.close(fig)
    ...
    if args.week3_figure:
        ...
        if not (theta and I1 and I0 and D and n1 and n0):
            raise SystemExit(
                "Provide --week3-theta --week3-I1 --week3-I0 --week3-D --week3-n1 --week3-n0"
            )
    ...
    if args.fh_heatmap:
        try:
            ...
            result = plot_fh_heatmap(
                rocA, rocB, comp="AND", add_anchors=True, outpath=str(heatmap_path)
            )
            print(
                f"✓ Wrote FH heatmap → {heatmap_path.resolve()}  (J_max={result['J_max']:.3f} at {result['argmax']})"
            )
