...
def compute_j_statistic(results: AttackResults) -> Tuple[float, float, float]:
    ...
    if not world_0 or not world_1:
        raise ValueError(
            f"Need results from both worlds. Got {len(world_0)} from world 0, {len(world_1)} from world 1"
        )
...
def bootstrap_ci_with_cc(
    ...
    if method == "percentile":
        ...
        ci_cc = (
            _percentile_ci(bootstrap_cc, alpha) if compute_cc and len(bootstrap_cc) > 0 else (0, 0)
        )
        ci_delta = (
            _percentile_ci(bootstrap_delta, alpha)
            if compute_cc and len(bootstrap_delta) > 0
            else (0, 0)
        )
    elif method == "bca":
    ...
    hypothesis_tests = _run_hypothesis_tests(
        j_emp,
        bootstrap_j,
        cc_max_emp if compute_cc else None,
        bootstrap_cc if compute_cc else None,
        alpha,
    )
...
def interpret_cc_results(
    ...
    if cc < threshold_constructive:
        ...
        confidence = (
            1.0
            if ci[1] < threshold_constructive
            else (threshold_constructive - cc) / (ci[1] - ci[0])
        )
        recommendation = "Deploy composition - synergistic protection detected"
    elif cc > threshold_destructive:
        regime = "destructive"
        confidence = (
            1.0 if ci[0] > threshold_destructive else (cc - threshold_destructive) / (ci[1] - ci[0])
        )
        recommendation = "Avoid composition - interference detected"
    else:
        regime = "independent"
        confidence = (
            1.0 if ci[0] > threshold_constructive and ci[1] < threshold_destructive else 0.5
        )
    ...
    return CCInterpretation(
        regime=regime, confidence=confidence, recommendation=recommendation, evidence=evidence
    )
...
def _bca_ci(
    samples: np.ndarray, theta_hat: float, original_data: AttackResults, alpha: float
) -> ConfidenceInterval:
...
def _run_hypothesis_tests(
    ...
    cc_emp: Optional[float],
    cc_bootstrap: Optional[np.ndarray],
    alpha: float,
) -> Dict[str, Any]:
