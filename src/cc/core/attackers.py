...
class RandomInjectionAttacker(AttackStrategy):
    ...
    def generate_attack(self, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        benign_tokens = self.rng.choice(self.vocab_benign, size=n_benign, replace=True) if n_benign > 0 else []
...
class GeneticAlgorithmAttacker(AttackStrategy):
    ...
    def _tournament_selection(self) -> List[str]:
        """Select individual via tournament selection"""
        tournament = self.rng.choice(len(self.population), size=min(self.tournament_size, len(self.population)), replace=False)
