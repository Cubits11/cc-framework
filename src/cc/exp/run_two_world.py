...
from ..core.models import GuardrailSpec, WorldConfig, AttackResult
from ..core.protocol import TwoWorldProtocol
from ..core.attackers import RandomInjectionAttacker, GeneticAlgorithmAttacker
from ..core.metrics import compute_j_statistic, compute_composability_coefficients
...
def analyze_results(results: List[AttackResult]) -> dict:
    ...
    bootstrap_result = bootstrap_ci_j_statistic(
        w0_array, w1_array, B=2000, alpha=0.05
    )
    ...
    analysis = {
        ...
        "success_rates": {
            "world_0_empirical": float(p0_hat),
            "world_1_empirical": float(p1_hat)
        },
        "j_statistic": {
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results/analysis.json', help='Output analysis file')
    ...
    try:
        ...
        # Print summary
        print("\n=== EXPERIMENT SUMMARY ===")
        print(f"J-statistic: {analysis['j_statistic']['empirical']:.4f} "
