...
def _plot_phase_surface(E: np.ndarray, T: np.ndarray, C: np.ndarray,
    ...
    ax.scatter([last_eps], [last_T], s=80, color=CBLUE, edgecolor="white", lw=0.8, zorder=3)
...
def _plot_roc_from_cfg(rec: Dict[str, Any], fig_path: Path) -> None:
    ...
    labelA = str(cfg.get("A", "A")); labelB = str(cfg.get("B", "B"))
    try:
        if io is None:
    ...
    ax.plot([0, 1], [0, 1], ls="--", lw=1.2, color="0.5", label="random")
