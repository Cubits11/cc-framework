    ...
    def build_guardrail_stack(self, specs: List[GuardrailSpec]) -> List[Guardrail]:
        ...
        for spec in specs:
            try:
                ...
                self.logger.log(
                    {"event": "guardrail_creation_failed", "name": spec.name, "error": str(e)}
                )
                raise
        ...
        try:
            for i in range(n_sessions):
                ...
                if i > 0 and i % 500 == 0:
                    attacker.reset()
                    self.logger.log(
                        {"event": "attacker_reset", "session": i, "reason": "periodic_reset"}
                    )
            ...
            self.logger.log(
                {
                    ...
                    "world_0_success_rate": (
                        world_0_successes / world_0_total if world_0_total > 0 else 0
                    ),
                    "world_1_success_rate": (
                        world_1_successes / world_1_total if world_1_total > 0 else 0
                    ),
...
__all__ = ["TwoWorldProtocol", "SessionMetadata", "ExperimentMetadata"]
