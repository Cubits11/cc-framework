...
from cc.analysis.generate_figures import plot_fh_heatmap, plot_cc_ci_comparison
from cc.cartographer.audit import audit_fh_ceiling_by_index, append_record, verify_chain
...
def main():
    ap = argparse.ArgumentParser(description="Week-3: generate FH heatmap + CC CI comparison, run auditor, append audit record.")
    ...
    ap.add_argument("--config-json", type=str, default=None, help="Optional JSON carrying the above CC fields")
    ...
    heat = plot_fh_heatmap(
        roc_a, roc_b,
        comp=args.comp, add_anchors=True,
        outpath=args.out_heatmap
    )
    print(f"[heatmap] J_max={heat['J_max']:.4f} at idx={heat['argmax']}; saved -> {heat['outpath']}")
    ...
    if missing:
        ...
        vals.update(dict(
            p1_hat=0.42, p0_hat=0.11, n1=500, n0=500, D=0.8,
            tpr_a=0.78, tpr_b=0.70, fpr_a=0.10, fpr_b=0.12
        ))
    ...
    print(f"[cc-cis] CĈ={ccfig['cc_hat']:.4f}  "
    ...
    j_cap = float(Jgrid[ia, ib]); j_obs = j_cap - 1e-15
    violations = audit_fh_ceiling_by_index(roc_a, roc_b, [(ia, ib, j_obs)], comp=args.comp, add_anchors=True)
    if violations:
    ...
    sha = append_record(
        ...
        cfg={"A": "railA", "B": "railB", "comp": args.comp, "samples": {"n1": int(vals["n1"]), "n0": int(vals["n0"])}, "seed": args.seed},
        j_a=0.0, j_a_ci=None,
        j_b=0.0, j_b_ci=None,
