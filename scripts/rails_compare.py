...
def sweep_thr_for_fpr(
    ...
    for thr in grid:
        ...
        if fpr_min <= fpr <= fpr_max:
            cand_w = SweepBest(thr=float(thr), j=float(jj), tpr=tpr, fpr=fpr, tnr=tnr, hit_window=True)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Compare two rails under an FPR window and write summary CSV.")
    p.add_argument("--csv", required=True, type=Path, help="Input CSV with columns: label, rail_a_score, rail_b_score")
    p.add_argument("--out", required=True, type=Path, help="Output CSV path (summary row).")
    p.add_argument("--fpr-min", type=float, default=0.0, help="Lower bound of FPR window (inclusive).")
    p.add_argument("--fpr-max", type=float, default=0.1, help="Upper bound of FPR window (inclusive).")
    args = p.parse_args()

    if not (0.0 <= args.fpr_min <= 1.0 and 0.0 <= args.fpr_max <= 1.0 and args.fpr_min <= args.fpr_max):
