...
@click.option(
    "--D", "D", type=float, required=True, help="Denominator (>0), D = min_r (1 - J_r(θ_r*))."
)
...
@click.option(
    ...
    help="Path to 0/1 series for Y=1 composite (A∧B).",
)
@click.option(
    "--y0-samples",
    type=click.Path(exists=True, dir_okay=False),
    help="Path to 0/1 series for Y=0 composite (A∨B).",
)
...
@click.option(
    ...
    default=None,
    help="If provided, save ROC+FH figure here (PNG).",
)
@click.option(
    "--json-out",
    type=click.Path(dir_okay=False),
    default=None,
    help="If provided, dump JSON with all numbers.",
)
def methods(
    D: float,
    tpr_a: float,
    ...
    if y1_samples or y0_samples:
        if not (y1_samples and y0_samples):
            raise click.BadParameter(
                "Provide both --y1-samples and --y0-samples when bootstrapping."
            )
        y1 = _read_binary_series(Path(y1_samples))
        y0 = _read_binary_series(Path(y0_samples))
        if len(y1) != n1 or len(y0) != n0:
            click.echo(
                "Warning: provided n1/n0 do not match sample lengths; using lengths from files."
            )
            n1 = len(y1)
            n0 = len(y0)
    ...
    if boo_lo is not None:
        click.echo(
            f"    Bootstrap   : [{boo_lo:.4f}, {boo_hi:.4f}]   (B={bootstrap_B}, seed={seed})"
        )
    else:
    ...
    if json_out:
        payload = {
            ...
            "bootstrap": {"lo": boo_lo, "hi": boo_hi, "B": bootstrap_B, "seed": seed}
            if boo_lo is not None
            else None,
        }
